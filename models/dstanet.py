...
class DSTANet(nn.Module):
    def __init__(self, num_class=20, num_point=13, num_frame=30, num_subset=3, dropout=0., config=None, num_person=1,
        ...
        self.config = [ 
            [64, 64, 16, 1], [64, 64, 16, 1],
            [64, 128, 32, 2], [128, 128, 32, 1],
            [128, 256, 64, 2], [256, 256, 64, 1],
            [256, 256, 64, 1], [256, 256, 64, 1],]

        self.out_channels = self.config[-1][1]
        in_channels = self.config[0][0]
        ...
        for index, (in_channels, out_channels, inter_channels, stride) in enumerate(self.config):
...
if __name__ == '__main__':
    ...
    net = DSTANet()  # .cuda()
    ske = torch.rand([2, 2, 30, 13, 1])  # .cuda()
    print(net(ske).shape)
